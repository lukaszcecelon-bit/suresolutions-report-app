...
const HOW_IT_WORKS = [
  ...
  },
  {
    ...
    body: 'iOS pokazuje w oknie udostępniania tylko te aplikacje, które zgłosiły systemowi, że przyjmują dany typ pliku — i tylko te włączone w rzędzie ikon. Jeśli brakuje Teams: przewiń rząd aplikacji do końca w prawo → „Więcej” → włącz Microsoft Teams (możesz je też przestawić na początek). Jeśli Teams nie ma na liście nawet po włączeniu w „Więcej”, i nie widać go też przy udostępnianiu dowolnego innego pliku (np. z aplikacji Pliki), to blokada jest po stronie telefonu albo firmowej polityki bezpieczeństwa Microsoft (Intune potrafi zabronić Teamsowi przyjmowania plików z aplikacji spoza polityki) — tego nie da się obejść z poziomu naszej aplikacji, to pytanie do administratora M365. Droga na skróty, która działa zawsze: „Zapisz w Plikach” → folder OneDrive, a w Teams załącz plik z OneDrive. Albo wyślij raport Outlookiem — przyjmuje załączniki bez problemu. Komunikat „Nie można przekazać pliku” w samym Teams to kwestia OneDrive/M365, nie aplikacji.',
  },
  {
    icon: '🌗',
